...
/* see man malloc */
/*
 * malloc searchs a free block big enough to hold a request of size 'size',
...
void *MALLOC_FUNCTION_EX (size_t size, char *block_ptr) {
  ...
  if (old_size < MIN_SIZE) {
    if (size & BYTES2TLSF_WORD_OFFSET_MASK)
      old_size ++;
    size = old_size;
    if (old_size <= 6) {
      fl = ptr_TLSF -> small_blocks_bitmap & ((~0) << (old_size - 2));
      if (fl && (fl = TLSF_fls(fl)) != -1)
	goto small_found;
    }
    fl = 0;
    sl = 0;
     ...
     * The block given is greater than the asked for size
     */
   ...
   */
  fl = TLSF_ffs(ptr_TLSF -> bitmapFL & ((~0) << (fl + 1)));
  
  if (fl > 0) {
    sl = TLSF_ffs(ptr_TLSF -> fl_array[fl].bitmapSL);
#ifdef SANITY_CHECK
  ...
  /*
   * we can say: YESSSSSSSSSSS, we have memory enough!!!!
   */
 
 small_found:
  THREAD_LOCK();
  bh = ptr_TLSF -> small_blocks_array [fl];
  remove_small_block (bh, ptr_TLSF);

  // Because of simplicity, small blocks are never splitted
  SET_USED_BLOCK(bh);
  
  THREAD_UNLOCK();
#ifdef SANITY_CHECK
  checking_structure (ptr_TLSF, "Leaving Malloc small");
#endif
