...
void FREE_FUNCTION_EX (void *ptr, char *block_ptr) {
  ...
  if (!IS_LAST_BLOCK (bh)) {
    ...
    if (!IS_USED_BLOCK (bh2)) {
      /* we are lucky, we can merge bh with the next one */
      // bh2 may not be in the normal TLSF structure
      if (GET_BLOCK_SIZE(bh2) >= MIN_SIZE)
	remove_block (bh2, ptr_TLSF);
      else
	remove_small_block (bh2, ptr_TLSF);
  ...
  if (aux_size >= MIN_SIZE){
    ...
    fl = aux_size - 2; 
    init_and_insert_small_block (ptr_TLSF, (block_header_t *)bh, fl);
  }
