...
static inline int check_small_fl (int fl, char *msg) {
  if (fl < 0 || fl > 5) {
    SANITY_PRINTF ("SANITY_CHECK error ([fl] %d): %s\n", fl, msg);
    return 1;
  }
  return 0;
}
...
static inline int checking_structure(TLSF_t *ptr, char *msg) {
  ...
  for (i = 0; i < 6; i++) {
    if ((ptr -> small_blocks_bitmap & (1 << i)) && 
	!ptr -> small_blocks_array [i]) {
      SANITY_PRINTF ("--> %s\n", msg);
      return -1;
    }
    if (ptr -> small_blocks_array [i] && 
	check_range_ptr ((char *) ptr -> small_blocks_array [i], 
			 "checking_structure")) {
      SANITY_PRINTF("12> %s\n",msg);
      return 1;
    }
    if (ptr -> small_blocks_array [i] != NULL) {
      b = (block_header_t *) ptr -> small_blocks_array [i];
  ...
  // OK, now physical contigous blocks are checked
